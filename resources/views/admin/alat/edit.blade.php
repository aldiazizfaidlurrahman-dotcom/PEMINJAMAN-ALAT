...
<div class="content-wrapper">
    ...
    <div style="margin-bottom: 1.5rem; font-size: 0.875rem; color: #64748b; display: flex; align-items: center; gap: 8px;">
        ...
        <span>Data Alat</span>
        <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"></path></svg>
        <span style="color: #1e293b; font-weight: 600;">Edit Alat</span>
    </div>

    <div class="form-card">
        <div class="form-header">
            <h3>Edit Informasi Alat</h3>
            <p>Perbarui detail teknis, kategori, atau jumlah stok alat yang terpilih.</p>
        </div>

        <form action="{{ route('admin.alat.update', $alat->id) }}" method="POST" id="editAlatForm">
            @csrf
            @method('PUT')
            ...
            <div class="grid">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="nama_alat" name="nama_alat" class="form-control" placeholder="Masukkan nama alat" value="{{ old('nama_alat', $alat->nama_alat) }}" required autofocus>
                    @error('nama_alat') <span class="error-text">{{ $message }}</span> @enderror
                </div>

                {{-- Kategori --}}
                <div class="form-group">
                    <label for="kategori_id">Kategori Alat</label>
                    <select id="kategori_id" name="kategori_id" class="form-control" required>
                        <option value="" disabled>-- Pilih Kategori --</option>
                        @foreach($kategori as $kat)
                            <option value="{{ $kat->id }}" @if(old('kategori_id', $alat->kategori_id) == $kat->id) selected @endif>{{ $kat->nama_kategori }}</option>
                        @endforeach
                    </select>
                ...
                {{-- Kondisi --}}
                <div class="form-group">
                    <label for="kondisi">Status Kondisi</label>
                    <select id="kondisi" name="kondisi" class="form-control" required>
                        ...
                        <option value="diperbaiki" @if(old('kondisi', $alat->kondisi) === 'diperbaiki') selected @endif>Diperbaiki</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="stok" name="stok" class="form-control" placeholder="0" value="{{ old('stok', $alat->stok) }}" min="0" required>
...
<script>
    // Loading Feedback
    document.getElementById('editAlatForm').onsubmit = function() {
